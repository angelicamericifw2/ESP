...
switch($act){
    ...
    case "INPUT_KARYAWAN":
    $KODEKARYAWAN = strtoupper($_POST['KODEKARYAWAN']);
    ...
    if(isset($_POST['NPWP'])){
        $NPWP = $_POST['NPWP'];
        $sql .= "SELECT * FROM ".$db." WHERE NPWP !='' AND NPWP =:NPWP";
    } else { $NPWP = null; }
    ...
    if($row = $stmt->fetch()){
        ...
        $sql = "SELECT * FROM ".$db." WHERE KODEPELANGGAN =:KODEPELANGGAN";
        $stmt = $pdo->prepare($sql);
        $stmt->execute(['KODEPELANGGAN'=>$KODEPELANGGAN]);
        if($row = $stmt->fetch()){
            $array['hasil'] = "ada";
    ...
    $pelanggan = explode("-", $KODEPELANGGAN);
    
    $kode_oto = range(100000,999999);
                
    $sql = "SELECT * FROM otorisasi";
    $stmt = $pdo->prepare($sql);
    $stmt->execute();
    while($row = $stmt->fetch()){
        $cek_nomorBukti = $row['NOMORBUKTI'];
        $find = 'RK';
        $pos = strpos($cek_nomorBukti, $find);
        if (($key = array_search($row['KODE'], $kode_oto)) !== false && $pos === false) {
            unset($kode_oto[$key]);
        }
    }
                         
    $random_key = array_rand($kode_oto,1);
                                
    $i=1;
                    
    $kode_oto = $kode_oto[$random_key];
                    
    foreach ($pelanggan as $key => $value)
    {
        if($value != null){
            $sql = "INSERT INTO otorisasi (KODE, NOMORBUKTI, KODESALESMAN, KODEPELANGGAN, DATETRANSACTION) VALUES (:KODE, :NOMORBUKTI, :KODESALESMAN, :KODEPELANGGAN, :DATETRANSACTION)";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([
                'KODE'=>$kode_oto,
                'NOMORBUKTI'=>$i,
    ...
    $row = $stmt->fetch();
    $inbox_isi = explode("<button", $row['isi']);
    
    $isi_pesan = $inbox_isi[0] ."<p>Authentication code :". $kode_oto ."</p>";
    
    $sql2 = "SELECT * FROM inbox_otorisasi WHERE INBOX=:INBOX";
    ...
    $sql2 = "SELECT * FROM inbox_otorisasi WHERE KODE=:KODE";
    $stmt2 = $pdo->prepare($sql2);
    $stmt2->execute([
        'KODE'=>$row2['KODE']
    ]);
    ...
    $inbox_id = $row2['INBOX'];
    $count = 0;
    $array1['KODE_found'][$count] = $inbox_id;
    $count++;
    $sql3 ="UPDATE inbox set isi=:isi WHERE id=:id";
    ...
    while($row2 = $stmt2->fetch()){
        $inbox_id = $row2['INBOX'];
        $array['KODE_found'][$count] = $inbox_id;
        $sql3 ="UPDATE inbox set isi=:isi WHERE id=:id";
        $stmt3 = $pdo->prepare($sql3);
        $stmt3->execute([
            'isi'=>$isi_pesan,
    ...
    case "INPUT_RENCANA_KUNJUNGAN_OTORISASI":
    $DATETRANSACTION = $_POST['DATETRANSACTION'];
    $USERID = $_POST['USERID'];
    ...
    try{
        ...
        if($row = $stmt->fetch()){
            ...
            foreach ($KODEPELANGGAN as $key => $value) {
                ...
                $sql2 ="UPDATE otorisasi set NOMORBUKTI=:NOMORBUKTI WHERE KODE=:KODE AND KODEPELANGGAN=:KODEPELANGGAN";
                $stmt2 = $pdo->prepare($sql2);
                $stmt2->execute([
                    'NOMORBUKTI'=>$NOMORBUKTI,
                    'KODE'=>$OTORISASI,
    ...
    try {
        ...
        $need_otorisasi = false;
        $pelanggan = "";
        $cek_user = "";
        foreach ($KODEPELANGGAN as $key => $value) {
            $pelanggan .= $value . "-";
            
            $sql = "SELECT * FROM tmrencanakunjungan WHERE KODEPELANGGAN=:KODEPELANGGAN AND DATETRANSACTION=:DATETRANSACTION";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([
                'DATETRANSACTION' => date('Y-m-d',strtotime($DATETRANSACTION)),
                'KODEPELANGGAN'=>$value
            ...
            if($stmt->rowCount() > 0 && $cek_user != $USERID ){
                $sql2 = "SELECT * FROM tmrencanakunjungan WHERE KODEPELANGGAN=:KODEPELANGGAN AND DATETRANSACTION=:DATETRANSACTION AND KODESALESMAN=:KODESALESMAN";
                $stmt2 = $pdo->prepare($sql2);
                $stmt2->execute([
                    'DATETRANSACTION' => date('Y-m-d',strtotime($DATETRANSACTION)),
                    'KODEPELANGGAN'=>$value,
                ...
                if($row2 = $stmt2->fetch()){
                    throw new Exception("Rencana kunjungan sudah diinputkan dengan kode pelanggan : ". $value);
                }
                else
                {
                    $need_otorisasi = true;
                    $detailRencana['NOMORBUKTI'][$i] = $row['NOMORBUKTI'];
                    $detailRencana['KODESALESMAN'][$i] = $row['KODESALESMAN'];
                    $detailRencana['KODEPELANGGAN'][$i] = $row['KODEPELANGGAN'];
                    $i++;
                    array_push($cekPelanggan, $row['KODEPELANGGAN']);
                    while($row = $stmt->fetch()){
                        $detailRencana['NOMORBUKTI'][$i] = $row['NOMORBUKTI'];
                        $detailRencana['KODESALESMAN'][$i] = $row['KODESALESMAN'];
                        $detailRencana['KODEPELANGGAN'][$i] = $row['KODEPELANGGAN'];
                        $i++;
                        if(!in_array($row['KODEPELANGGAN'], $cekPelanggan))
                        {
                            array_push($cekPelanggan, $row['KODEPELANGGAN']);
                        }
                    }
                ...
                $sql3 = "SELECT * FROM otorisasi WHERE DATETRANSACTION=:DATETRANSACTION AND KODEPELANGGAN=:KODEPELANGGAN AND KODESALESMAN=:KODESALESMAN";
                $stmt3 = $pdo->prepare($sql3);
                $stmt3->execute([
                    'DATETRANSACTION'=>date('Y-m-d',strtotime($DATETRANSACTION)),
                    'KODEPELANGGAN'=>$value,
                ...
                $warning['KODE'] = $row3['KODE'];
                   
            }
            
        }
        if($warning['KODE'] == null)
        {
            $sql = "SELECT * FROM tmrencanakunjungan WHERE DATETRANSACTION=:DATETRANSACTION AND NOMORBUKTI LIKE 'RK%' ORDER BY NOMORBUKTI DESC";
            $stmt = $pdo->prepare($sql);
            $stmt->execute(['DATETRANSACTION'=>date('Y-m-d',strtotime($DATETRANSACTION))]);
            if($row = $stmt->fetch()){
                ...
                if($need_otorisasi == true)
                {
                    $judul = "PENGAJUAN JOIN VISIT";
                    $jenis ="notification_support";
                    $isi = "Sales ingin membuat rencana kunjungan :<br>";
                    foreach ($cekPelanggan as $cp => $value)
                    {
                        $isi .= "<b>". $value . "</b><br>";
                        for($x = 0; $x < $i; $x++)
                            {
                            if($detailRencana['KODEPELANGGAN'][$x] == $value)
                            {
                                $isi .= "- ". $detailRencana['NOMORBUKTI'][$x] ."(". $detailRencana['KODESALESMAN'][$x] .")<br>";
                            }
                        }
                        $isi .="<br>";
                    }
                    ...
                    $penerima = array();
                    $sql = "SELECT * FROM pemakai WHERE KODEJABATAN = 'OWNER'OR KODEJABATAN = 'ADMIN'";
                    $stmt = $pdo->prepare($sql);
                    $stmt->execute();
                    if($stmt->rowCount() > 0){
                        $id = 0;
                        while($pemakai = $stmt->fetch()){
                            $penerima['KODEKARYAWAN'][$id] = $pemakai['USERID'];
                            $id++;
                        }
                    }
                    
                        $sql3 = "SELECT * FROM inbox_otorisasi ORDER BY KODE DESC LIMIT 1";
                        $stmt3 = $pdo->prepare($sql3);
                        $stmt3->execute();
                        if($row = $stmt3->fetch()){
                            $KODE = $row['KODE'] + 1;
                    ...
                    $karyawan_i = 0;        
                    foreach ($penerima['KODEKARYAWAN'] as $key => $value) {
                        ...
                        $sql = "INSERT INTO inbox_otorisasi (KODE, INBOX) values(:KODE,:INBOX)";
                        $stmt = $pdo->prepare($sql);
                        $stmt->execute([
                            'KODE'=>$KODE,
                            'INBOX'=>$inbox_id
                        ...
                        $sql4 = "SELECT * FROM inbox WHERE id=:id";
                        ...
                        $row = $stmt4->fetch();
                        $pesan = $row['isi']. "<button class='col-30 button button-fill' name='buttonKode' onclick='kirimOtorisasi(`".$inbox_id."`,`".$USERID."`,`".$pelanggan."`,`".$DATETRANSACTION."`)'>Setujui</button><br><p name='cetak_kode'></p>";
                        ...
                        $warning['SQL_INSERT'][$karyawan_i] = $inbox_id;
                        $warning['SQL_UPDATE'][$karyawan_i] = $pesan;
                        $karyawan_i++;
                    }

                    $warning['pesan']="Hubungi admin untuk mendapatkan kode.";
    ...
    try{
        ...
        if($KODECABANG == "SOLO")
        {
            $sql = "SELECT mp.USERID as KODESALESMAN, mp.KODEPELANGGAN as KODEPELANGGAN, mp.NAMA as NAMA FROM mpelanggansolo mp";
            $stmt = $pdo->prepare($sql);
            $stmt->execute();
            $id = 0;
            if($stmt->fetch() != null)
    ...
    if($halaman_hak_akses == "EDIT_USER_HAK_AKSES"){
        ...
        $sql ="DELETE FROM usermenu1 WHERE USERID =:USERID";
        $stmt = $pdo->prepare($sql);
        $stmt->execute(['USERID'=>$USERID]);
        ...
        for($i = 0; $i<count($SCREEN); $i++){
            ...
            $sql = "SELECT * FROM menu WHERE SCREEN =:SCREEN";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([
                'SCREEN'=>$SCREEN[$i]
            ]);
            ...
            $sql = "SELECT * FROM menu1 WHERE NOMOR =:NOMOR";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([
                'NOMOR'=>$new_nomor
            ]);
            ...
            $new_screen = "";
            if($row = $stmt->fetch()){
                $new_screen = $row['SCREEN'];
            }
            
            $sql = "INSERT INTO usermenu1 (SCREEN,USERID,TAMBAH,UBAH,HAPUS,CETAK,POSTING,CARI) values(:SCREEN,:USERID,:TAMBAH,:UBAH,:HAPUS,:CETAK,:POSTING,:CARI)";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([
                'SCREEN'=>$new_screen,
                'USERID'=>$USERID,
        ...
        for($i = 0; $i<count($SCREEN); $i++){
            ...
            $sql = "SELECT * FROM menu1 WHERE NOMOR =:SCREEN";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([
                'SCREEN'=>$SCREEN[$i]
            ]);
    ...
    if(isset($KATEGORIPELANGGAN) && isset($KJ1))
    {
        $VERGPS = "T";
    }
    ...
    $array['WAKTUKUNJUNGAN'] = array();
    
    $sql = "SELECT TIME(WAKTUMASUK) as WAKTU FROM tmkunjungan WHERE WAKTUMASUK LIKE '%".date("Y-m-d",strtotime($DATETRANSACTION))."%' AND KODESALESMAN ='".$KODESALESMAN."' ORDER BY WAKTUMASUK ASC LIMIT 1";

    $stmt = $pdo->prepare($sql);
    $stmt->execute();
    $start_kunjungan = $stmt->fetch(); 
    $array['start'] = $start_kunjungan['WAKTU'];
    ...
    if(isset($array['NOMORBUKTI'])){
        for($i = 0; $i <count($array['NOMORBUKTI']); $i++){
            ...
            if($kunjungan = $stmt->fetch()){
                ...
                $array['WAKTUKUNJUNGAN'][$i] = $kunjungan['DATETRANSACTION'];
            } else {
    ...
    if($kunjungan = $stmt->fetch()){
        ...
        $array['KESIMPULAN'][$id] = $kunjungan['KESIMPULAN'];
        $array['PERMINTAANKHUSUS'][$id] = $kunjungan['PERMINTAANKHUSUS'];
        $array['WAKTUKUNJUNGAN'][$id] = $kunjungan['WAKTUKUNJUNGAN'];
        $id++;
        while($kunjungan = $stmt->fetch()){
            $array['NOMORBUKTI'][$id] = $kunjungan['NOMORBUKTI'];
            $array['KODESALESMAN'][$id] =$kunjungan['KODESALESMAN'];
    ...
    $sql = "SELECT tk.STATUSKUNJUNGAN AS STATUSKUNJUNGAN, tk.NOMORBUKTI as NOMORBUKTIKUNJUNGAN, mk.NAMA as NAMAKARYAWAN,mpel.NAMA as NAMA, mpel.KODEPELANGGAN as KODEPEL, mpel.ALAMAT as ALAMATPEL, mpel.KOTA as KOTAPEL,mpel.TELPON as TELPONPEL,mpel.FAX as FAXPEL,mpel.KREDITLIMIT as KREDITLIMITPEL, mpel.SYARATPIUTANG as SYARATPIUTANGPEL, mpel.SYARATPIUTANGMAX as SYARATPIUTANGMAXPEL, mpel.USERID as USERIDPEL, mpel.TERM as TERMPEL, mpel.KATEGORIPELANGGAN as KATEGORIPELANGGANPEL, mpel.ALAMATKIRIM as ALAMATKIRIMPEL, tk.WAKTUMASUK AS JAMMASUK,tk.DATETRANSACTION AS JAMREPORT,tk.KETERANGAN as KETERANGAN, tk.KESIMPULAN as KESIMPULAN, tk.PERMINTAANKHUSUS as PERMINTAANKHUSUS FROM tmkunjungan tk INNER join ".$mpel." as mpel on(mpel.KODEPELANGGAN = tk.KODEPELANGGAN) inner join mkaryawan mk on(mk.KODEKARYAWAN = tk.KODESALESMAN) inner join mpermintaankhusus mp on(mp.NAMA = tk.PERMINTAANKHUSUS) WHERE mpel.KODECABANG =:KODECABANG";
    ...
    if($row = $stmt->fetch()){
        $array['hasil'] = "ada";
        $array['NAMA'][$id] = $row['NAMA'];
        $array['KODEPELANGGAN'][$id] = $row['KODEPEL'];
        $array['ALAMAT'][$id] = $row['ALAMATPEL'];
        $array['KOTA'][$id] = $row['KOTAPEL'];
        $array['TELPON'][$id] = $row['TELPONPEL'];
        $array['FAX'][$id] = $row['FAXPEL'];
        $array['KREDITLIMIT'][$id] = $row['KREDITLIMITPEL'];
        $array['SYARATPIUTANG'][$id] = $row['SYARATPIUTANGPEL'];
        $array['SYARATPIUTANGMAX'][$id] = $row['SYARATPIUTANGMAXPEL'];
        $array['USERID'][$id] = $row['USERIDPEL'];
        $array['TERM'][$id] = $row['TERMPEL'];
        $array['KATEGORIPELANGGAN'][$id] = $row['KATEGORIPELANGGANPEL'];
        $array['ALAMATKIRIM'][$id] = $row['ALAMATKIRIMPEL'];
    ...
    if(isset($array['NOMORBUKTIKUNJUNGAN'])){
        ...
        for($i = 0;$i<count($array['NOMORBUKTIKUNJUNGAN']);$i++){
            ...
            if($array['PERMINTAANKHUSUS'][$counter] == "FOLLOW UP BY PHONE" )
            {
                $jum_follow++;
            }
            else
        ...
        $array['JUMKUNJUNGAN'] = $jum_kunjungan;
        $array['JUMFOLLOW'] = $jum_follow;
    }
    ...
    $sql = "SELECT * FROM mpelanggan mp INNER JOIN dpelanggan1 dp on(dp.KODEPELANGGAN = mp.KODEPELANGGAN) WHERE LATITUDE IS NOT NULL AND LONGITUDE IS NOT NULL AND dp.KJ1 = 'SALESMAN' AND VERGPS = :VERGPS GROUP BY dp.KODEPELANGGAN ORDER BY KATEGORIPELANGGAN ASC";
    $stmt = $pdo->prepare($sql);
    $stmt->execute(['VERGPS'=>'F']);
    $array = array();
    $id = 0;
    if($KOORDINAT = $stmt->fetch()){
        $array['hasil'] = "ada";
    ...
    if(!isset($_POST['KODEKATEGORIPELANGGAN']) || empty($_POST['KODEKATEGORIPELANGGAN'])){
        $KODEKATEGORIPELANGGAN = null;
    } else {
        $KODEKATEGORIPELANGGAN = $_POST['KODEKATEGORIPELANGGAN'];
    }
    ...
    if(!isset($KODEKATEGORIPELANGGAN)){
        $sql = "SELECT * FROM mpelanggan mp INNER JOIN dpelanggan1 dp on (dp.KODEPELANGGAN = mp.KODEPELANGGAN) INNER JOIN pemakai p on (p.USERID = dp.CP1) WHERE LATITUDE IS NOT NULL AND LONGITUDE IS NOT NULL AND p.KODEJABATAN = 'SALESMAN' AND p.USERID = :KODESALESMAN";
        $sql.= " AND VERGPS =:VERIFIKASI GROUP BY mp.KODEPELANGGAN ORDER BY KATEGORIPELANGGAN,mp.NAMA ASC ";
        $stmt = $pdo->prepare($sql);
        $stmt->execute(['VERIFIKASI'=>'T','KODESALESMAN'=>$KODESALESMAN]);
    } else {
        $sql = "SELECT * FROM mpelanggan mp INNER JOIN dpelanggan1 dp on (dp.KODEPELANGGAN = mp.KODEPELANGGAN) INNER JOIN pemakai p on (p.USERID = dp.CP1) WHERE LATITUDE IS NOT NULL AND LONGITUDE IS NOT NULL AND VERGPS =:VERIFIKASI AND KATEGORIPELANGGAN = :KODEKATEGORIPELANGGAN AND dp.KJ1 = 'SALESMAN' AND dp.CP1 = :KODESALESMAN GROUP BY mp.KODEPELANGGAN ORDER BY KATEGORIPELANGGAN,mp.NAMA ASC ";
        $stmt = $pdo->prepare($sql);
        $stmt->execute(['KODEKATEGORIPELANGGAN'=>$KODEKATEGORIPELANGGAN,'VERIFIKASI'=>$VERIFIKASI,'KODESALESMAN'=>$KODESALESMAN]);
    } 
    ...
    if(empty($_POST['KODEKATEGORIPELANGGAN'])){
        $KODEKATEGORIPELANGGAN = null;
    } else {
        $KODEKATEGORIPELANGGAN = $_POST['KODEKATEGORIPELANGGAN'];
    }
    ...
    if(empty($KODEKATEGORIPELANGGAN)){
        $sql = "SELECT * FROM mpelanggan mp INNER JOIN dpelanggan1 dp on (dp.KODEPELANGGAN = mp.KODEPELANGGAN) INNER JOIN pemakai p on (p.USERID = dp.CP1) WHERE LATITUDE IS NOT NULL AND LONGITUDE IS NOT NULL AND p.KODEJABATAN = 'SALESMAN' AND p.USERID = :KODESALESMAN";
        $sql.= " AND VERGPS =:VERIFIKASI GROUP BY mp.KODEPELANGGAN ORDER BY KATEGORIPELANGGAN,mp.NAMA ASC ";
        $stmt = $pdo->prepare($sql);
        $stmt->execute(['VERIFIKASI'=>'T','KODESALESMAN'=>$KODESALESMAN]);
                ...
                if($stmt2->rowCount() > 0){
                    while($point2 = $stmt2->fetch()){
                        if(date('Y-m-d',strtotime($point2['CREATED_AT'])) < date('Y-m-d',strtotime($point2['DATETRANSACTION_RENCANA']))){
                            $besar_point = 0;
                            $sql = "SELECT KATEGORIPELANGGAN FROM mpelanggan WHERE KODEPELANGGAN LIKE '%" . $point['KODEPELANGGAN'] . "%'";
                                $stmt_pelanggan = $pdo->prepare($sql);
                                $stmt_pelanggan->execute();
                                $pelanggan = $stmt_pelanggan->fetch();

                                    if (stripos($pelanggan['KATEGORIPELANGGAN'], "TOKO") !== FALSE) {
                                        $besar_point = 2;
                                    }
                                    if (stripos($pelanggan['KATEGORIPELANGGAN'], "FLEET") !== FALSE) {
                                        $besar_point = 3.2;
                                    }
                                    if(stripos($pelanggan['KATEGORIPELANGGAN'], "FLEET") === FALSE && stripos($pelanggan['KATEGORIPELANGGAN'], "TOKO") === FALSE)
                                    {
                                        $besar_point = 0;
                                    }
                                $array['total_point_kunjungan'] += $besar_point;
    ...
    try {
        ...
        $sql = "SELECT  tm.KODEPELANGGAN AS KODEPELANGGAN,date(tm.DATETRANSACTION) AS DATETRANSACTION, tm.NOMORBUKTI AS NOMORBUKTI, tm.KODESALESMAN AS KODESALESMAN, mp.NAMA AS NAMAPELANGGAN, mp.KOTA AS KOTA, mp.KATEGORIPELANGGAN AS KATEGORIPELANGGAN FROM tmkunjungan tm INNER JOIN mpelanggan mp on(mp.KODEPELANGGAN = tm.KODEPELANGGAN) WHERE tm.NOMORBUKTI LIKE 'KS%' AND ";
        if($STAFFDARI){
            $sql .= " tm.KODESALESMAN >= '" . $STAFFDARI ."' AND ";
        }
        ...
        if($stmt->rowCount() > 0){
            ...
            while($rencana = $stmt->fetch()){
                $array['KUNJUNGAN'][$id] = [
                    ...
                    'KOTA'=>$rencana['KOTA'],
                    'KODEPELANGGAN'=>$rencana['KODEPELANGGAN'],
                    'KATEGORIPELANGGAN'=>$rencana['KATEGORIPELANGGAN']
    ...
    try {
        ...
        $message = 'Tanggal : '.date('Y-m-d H:i:s').' Error :' . $e->getMessage();
        $sql = "INSERT INTO debug (userid,keterangan) values(:USERID,:ex)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([
            'USERID'=>$USERID,
            'ex'=>$message
    ...
    case "LAPORAN_POINT_KUNJUNGAN":
        ...
        for($i = 0; $i<=$numberDays; $i++){
            $sql = "SELECT mp.NAMA AS NAMAPELANGGAN, tk.KODEPELANGGAN AS KODEPELANGGAN, tk.DATETRANSACTION AS DATETRANSACTION_KUNJUNGAN, tk.NOMORBUKTI AS NOMORBUKTI_KUNJUNGAN FROM tmkunjungan tk INNER JOIN mpelanggan mp on(mp.KODEPELANGGAN = tk.KODEPELANGGAN) WHERE tk.DATETRANSACTION >'".date('Y-m-d 00:00:00',strtotime('+'.$i.'days',strtotime($DARITANGGAL)))."' AND tk.DATETRANSACTION<'".date('Y-m-d 23:59:59',strtotime('+'.($i+1).'days',strtotime($DARITANGGAL)))."' AND tk.KODESALESMAN =:KODESALESMAN ORDER BY tk.DATETRANSACTION ASC";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([
                'KODESALESMAN'=>$KODESALESMAN
            ]);
                ...
                while($point = $stmt->fetch()){
                    ...
                    if($stmt2->rowCount() > 0){
                        ...
                        while($point2 = $stmt2->fetch()){
                            ...
                            if(date('Y-m-d',strtotime($point2['CREATED_AT'])) < date('Y-m-d',strtotime($point2['DATETRANSACTION_RENCANA']))){
                                $sql = "SELECT KATEGORIPELANGGAN FROM mpelanggan WHERE KODEPELANGGAN =:KODEPELANGGAN";
                                ...
                                $array['laporan_point_kunjungan'][$id] = [
                                    ...
                                    'KATEGORIPELANGGAN'=>$pelanggan['KATEGORIPELANGGAN'],
    ...
    if($jabatan == "OWNER" || $jabatan == "ADMIN" || $jabatan == "FINANCE"){
        ...
        $sql = "SELECT * FROM mpengeluaran WHERE kodesalesman=:kodesalesman";
        $stmt = $pdo->prepare($sql);
    ...
    try {
        $menu = array();
        /*if($params['USERID'] == "SUMBER"){*/
            $sql = "SELECT * FROM usermenu1 um INNER JOIN menu1 m on(m.SCREEN = um.SCREEN) WHERE USERID = :USERID";
        /*} else {
            $sql = "SELECT * FROM usermenu um INNER JOIN menu m on(m.SCREEN = um.SCREEN) WHERE USERID = :USERID";
        }*/
        $stmt = $pdo->prepare($sql);
        $stmt->execute([
        ...
        if($stmt->rowCount() > 0){
            $array['hasil'] = "ada";
            $array['usermenu'] = $stmt->fetchAll(PDO::FETCH_CLASS);
            if($params['level'] == 1){
                $id = 0;
                foreach ($array['usermenu'] as $key => $value) {
                    if(!in_array(substr($value->SCREEN,0,1),$menu)){
                        $menu[$id] = substr($value->SCREEN, 0, 1);
                        $id++;
                    }
                }
            } else {
                ...
                foreach ($array['usermenu'] as $key => $value) {/*
                    echo $params['screen'];
                    echo substr($value->SCREEN, 0, $params['level'] -1);*/
                    /*if(!in_array(substr($value->SCREEN,0,$params['level']),$menu) && substr($value->SCREEN, 0, $params['level'] -1) == $params['screen']){
                        $menu[$id] = substr($value->SCREEN, 0, $params['level']);
                        $id++;
                    }*/
                    if($params['status'] == 'naik'){
                        if(!in_array(substr($value->SCREEN,0,$params['level']),$menu) && substr($value->SCREEN, 0, $params['level'] -1) == $params['screen']){
                            $menu[$id] = substr($value->SCREEN, 0, $params['level']);
                            $id++;
                        }
                    } else {
                        if((!in_array(substr($value->SCREEN,0,$params['level']),$menu) && strlen($params['screen']) == strlen($value->SCREEN)) && substr($value->SCREEN,0,$params['level']-1) == substr($params['screen'],0,$params['level']-1) ){
                            $menu[$id] = substr($value->SCREEN, 0, $params['level']);
                            $id++;
                        }
                    }
                ...
                if(strlen($params['screen']) >1 && $params['status'] == "turun"){
                    $menu[$id] = $params['screen'];
                    $id++;
                }
            }
        } else {
            $array['hasil'] = "tidakada";
            $array['message'] = "Tidak Ada Menu";
        }

        if(isset($menu) && (isset($params['status']) && $params['status'] == "turun")){
            /*if($params['USERID'] == "SUMBER"){*/
                $sql = "SELECT * FROM usermenu1 um INNER JOIN menu1 m on(m.SCREEN = um.SCREEN) WHERE USERID = :USERID AND um.SCREEN LIKE :SCREEN";
            /*}*/
            
            $stmt = $pdo->prepare($sql);
            $stmt->execute([
                'USERID'=>$params['USERID'],
                'SCREEN'=>substr($params['screen'],0,$params['level']-1) .'%'
            ]);
            if($stmt->rowCount() > 0){
                $array['usermenu'] = $stmt->fetchAll(PDO::FETCH_CLASS);
                $id++;
                foreach($array['usermenu'] as $key => $value){
                    if((!in_array(substr($value->SCREEN,0,$params['level']),$menu) )){
                        $menu[$id] = substr($value->SCREEN, 0, $params['level']);
                        $id++;
                    }
                }
        ...
        if(isset($menu) ){
            /*if($params['USERID'] == "SUMBER"){*/
                $sql = "SELECT * FROM menu1 WHERE SCREEN IN (".implode(',', $menu).")";
            /*} else {
                $sql = "SELECT * FROM menu WHERE SCREEN IN (".implode(',', $menu).")";
            }*/
            $stmt = $pdo->prepare($sql);
            $stmt->execute();
            $array['usermenu'] = $stmt->fetchAll(PDO::FETCH_CLASS);
        ...
        function sort_screen($a, $b) {
            return strcmp($a->SCREEN, $b->SCREEN);
        }

        usort($array['usermenu'], "sort_screen");

        foreach ($array['usermenu'] as $key => $value) {
            $array['usermenu'][$key]->NOMOR = utf8_encode($value->NOMOR);
            $array['usermenu'][$key]->SCREEN = utf8_encode($value->SCREEN);
            $array['usermenu'][$key]->MENU = utf8_encode($value->MENU);
            $array['usermenu'][$key]->ICON = utf8_encode($value->ICON);
            $array['usermenu'][$key]->ICON_GAMBAR = utf8_encode($value->ICON_GAMBAR);
            $array['usermenu'][$key]->TAG = utf8_encode($value->TAG);
            $array['usermenu'][$key]->HALAMAN = utf8_encode($value->HALAMAN);
            $array['usermenu'][$key]->HALAMAN_GLOBAL = utf8_encode($value->HALAMAN_GLOBAL);
        }
    ...
    try {
        ...
        $stmt = $pdo->prepare($sql);
        $stmt->execute();
        $array['menu'] = $stmt->fetchAll(PDO::FETCH_CLASS);
        foreach ($array['menu'] as $key => $value) {
            $array['menu'][$key]->NOMOR = utf8_encode($value->NOMOR);
            $array['menu'][$key]->SCREEN = utf8_encode($value->SCREEN);
            $array['menu'][$key]->MENU = utf8_encode($value->MENU);
            $array['menu'][$key]->ICON = utf8_encode($value->ICON);
            $array['menu'][$key]->TAG = utf8_encode($value->TAG);
            $array['menu'][$key]->HALAMAN = utf8_encode($value->HALAMAN);
            $array['menu'][$key]->HALAMAN_GLOBAL = utf8_encode($value->HALAMAN_GLOBAL);
        }
    } catch (Exception $e){
        $array['hasil'] = "gagal";
        $array['message'] = $e->getMessage();
    ...
    $array = array();
    try {
        $sql = "SELECT * FROM menu1 WHERE NOMOR = :nomor AND SCREEN = :screen";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([
            'nomor'=>$params['nomor'],
    ...
    try {
        $pdo->beginTransaction();
        $sql = "SELECT * FROM menu1 WHERE SCREEN = :SCREEN";
        $stmt = $pdo->prepare($sql);
        $stmt->execute(['SCREEN'=>$params['data']['SCREEN']]);
        if($stmt->rowCount() > 0){
            throw new Exception("Screen Sudah Terpakai");
        }
        $sql = "UPDATE menu1 set SCREEN =:SCREEN WHERE NOMOR = :NOMOR";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([
            'SCREEN'=>$params['data']['SCREEN'],
            'NOMOR'=>$params['data']['NOMOR']
        ]);

        $sql = "UPDATE usermenu1 set SCREEN = :SCREEN WHERE SCREEN = :SCREEN_LAMA";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([
            'SCREEN' => $params['data']['SCREEN'],
            'SCREEN_LAMA' => $params['data']['SCREEN_LAMA']
        ]);
        $array['hasil'] = "berhasil";
        $pdo->commit();
    ...
    case "TAMPIL_SEMUA_PIUTANG":
    $array = array();
    try {
        $db = "(DESCRIPTION=(ADDRESS_LIST = (ADDRESS = (PROTOCOL = TCP)(HOST = 122505214253.ip-dynamic.com)(PORT = 1521)))(CONNECT_DATA=(SID=BOND)))" ;

        if($c = OCILogon("smart", "301078", $db))
        {
            $array['hasil'] = "berhasil";
            $stid = oci_parse($c, 'SELECT * FROM mstokmichelin ');
            oci_execute($stid);
            $array['message'] = "Connection Berhasil";
            OCILogoff($c);
        }
        else{
            $err = OCIError();
            $array['message'] = "Connection failed." . $err[text]; 
        }
    } catch (Exception $e){
